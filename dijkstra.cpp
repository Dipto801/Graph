...
void dijkstra(ll s, ll n){
  // s=source, n=jei node a jabo
  for(ll i=0;i<=n;i++) dis[i]=1e18; // distance max korlam
  dis[s]=0; // source to source distance
  priority_queue<pll,vector<pll>,greater<pll>>pq; // small to big sort

  pq.push({0,s}); // first weight,then node

  while(!pq.empty()){
    ll u=pq.top().S;// node
    ll curD=pq.top().F; // distance
    pq.pop();

    if(dis[u]<curD) continue; //karon relaxation hobena
